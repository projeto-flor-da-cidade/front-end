...
export default function TelaDeDescricaoDeSolicitacaoHortas() {
  ...
  const [currentHortaImage, setCurrentHortaImage] = useState(LOCAL_FALLBACK_IMAGE);

  const fetchHortaDetails = useCallback(async () => {
    if (!hortaId) {
      toast.error("ID da horta não fornecido para descrição.");
      navigate(-1);
      setCurrentHortaImage(LOCAL_FALLBACK_IMAGE);
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      if (response.data) {
        const fetchedHorta = response.data;
        setHorta(fetchedHorta);

        let imageUrlToSet = LOCAL_FALLBACK_IMAGE;
        if (fetchedHorta.imagemCaminho) {
          const imageName = fetchedHorta.imagemCaminho.trim();
          if (imageName !== "" && imageName !== "folhin.png") {
            imageUrlToSet = `${BACKEND_URL}/uploads/imagem/${imageName}`;
          } else if (imageName === "folhin.png") {
            imageUrlToSet = `${BACKEND_URL}/uploads/imagem/folhin.png`;
          }
        }
        setCurrentHortaImage(imageUrlToSet);
  ...
  const handleApiAction = async (newStatus) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (newStatus === 'ATIVA') {
          navigate('/app/tela-de-solicitacao-hortas');
        } else {
          if (location.state?.source === 'solicitacoes') {
            navigate('/app/tela-de-solicitacao-hortas');
          } else {
            navigate('/app/tela-hortas-ativas'); 
          }
        }
      }, 2000);
    } catch (_err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-poppins pt-10 sm:pt-16 pb-10">
          ...
          <aside className="space-y-6 lg:sticky lg:top-20 self-start">
            <Section title="Imagem da Horta" icon={FiImage}>
              ...
              >
                ...
                />
                {/* 
                  Temporarily commented out the overlay div to isolate the image display problem.
                  If the image appears now, the issue is with this overlay or its interaction.
      ...
      {isImageModalOpen && currentHortaImage && (
        <div

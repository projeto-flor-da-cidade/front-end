...
const Section = ({ title, icon: Icon, children }) => ( <section className="bg-white p-5 rounded-xl shadow-lg"><h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Icon className="mr-2.5 text-green-600 w-5 h-5" /> {title}</h2>{children}</section> );
const InfoGrid = ({ children, cols = 2 }) => ( <div className={`grid grid-cols-1 sm:grid-cols-${cols} gap-x-5 gap-y-3 text-sm`}>{children}</div> );
const InfoItem = ({ label, value, href, children }) => ( <div><strong className="text-gray-500 font-medium">{label}:</strong>{' '}{children || (href ? <a href={href} className="text-blue-600 hover:underline break-all" target="_blank" rel="noopener noreferrer">{value || 'N/A'}</a> : <span className="text-gray-800 break-words">{value || 'N/A'}</span>)}</div> );
const ActionButton = ({ icon: Icon, text, onClick, isSubmitting, color = 'gray', href }) => { const classes = `w-full flex items-center justify-center gap-2 px-4 py-2 text-white rounded-md transition-colors text-sm font-medium disabled:opacity-70 ${{blue: 'bg-blue-600 hover:bg-blue-700',green: 'bg-green-600 hover:bg-green-700',red: 'bg-red-600 hover:bg-red-700',whatsapp: 'bg-green-500 hover:bg-green-600',gray: 'bg-gray-600 hover:bg-gray-700'}[color]}`; const content = (<>{isSubmitting ? <FiLoader className="animate-spin w-5 h-5" /> : <Icon className="w-5 h-5" />}{text}</>); return href ? <a href={href} target="_blank" rel="noopener noreferrer" className={classes}>{content}</a> : <button onClick={onClick} disabled={isSubmitting} className={classes}>{content}</button>;};
...
export default function TelaDeDescricaoDeSolicitacaoHortas() {
  ...
  if (isLoading) return ( <div className="flex flex-col items-center justify-center min-h-screen"><FiLoader className="animate-spin text-4xl text-green-600" /><p className="mt-3">Carregando...</p></div> );
  if (error || !horta) return ( <div className="flex flex-col items-center justify-center min-h-screen text-center p-4"><FiAlertCircle className="text-4xl text-red-500 mb-3" /><p className="font-semibold">{error || "Horta não encontrada."}</p><button onClick={() => navigate(-1)} className="mt-4 px-4 py-2 bg-gray-600 text-white rounded">Voltar</button></div> );
    ...
    <div className="min-h-screen bg-gray-100 font-poppins pt-10 sm:pt-16 pb-10">
      ...
      <div className="container mx-auto px-3 sm:px-4">
        <header className="mb-8 pb-4 text-center md:text-left border-b border-gray-200">
          ...
          <p className="text-xs text-gray-500 uppercase">Solicitação #{horta.idHorta}</p>
          <h1 className="text-3xl font-bold text-green-700 mt-1 break-words">{horta.nomeHorta}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <main className="lg:col-span-2 space-y-6">
            ...
            <Section title="Grupo e Atividades" icon={FiUsers}>
              <InfoGrid cols={1}><InfoItem label="Nº de Pessoas" value={horta.qntPessoas} /><div className="mt-2"><strong className="text-gray-600 block mb-1 text-sm">Atividades Planejadas:</strong><p className="text-gray-700 bg-gray-50 p-3 rounded-md border whitespace-pre-wrap">{horta.atividadeDescricao || 'N/A'}</p></div></InfoGrid>
            </Section>
          </main>
          ...
          <aside className="lg:sticky lg:top-20 self-start space-y-6">
            ...
            <Section title="Gerenciamento" icon={FiEdit}>
              <ActionButton icon={FiEdit} text="Editar Dados" onClick={() => navigate(`/app/hortas-editar/${hortaId}`)} color="gray" />
            </Section>
          </aside>
        </div>
      </div>

// Caminho: seu-projeto-frontend/src/pages/TelaGerenciamentoHortas.jsx
...
export default function TelaGerenciamentoHortas() {
    ...
    const fetchAllManageableHortas = useCallback(async () => {
        ...
        try {
            ...
            const uniqueHortas = Array.from(new Set(combinedHortas.map(a => a.idHorta)))
                .map(id => combinedHortas.find(a => a.idHorta === id));
    ...
    const handleStatusChange = async (hortaId, newStatusValue) => {
        const hortaToUpdate = allFetchedHortas.find(h => h.idHorta === hortaId);
        if (!hortaToUpdate) return;
        const originalStatus = hortaToUpdate.statusHorta;
        setActiveItemDropdownId(null);
        
        setAllFetchedHortas(prevHortas =>
            prevHortas.map(h => (h.idHorta === hortaId ? { ...h, statusHorta: newStatusValue } : h))
        );
        ...
        try {
            ...
            setAllFetchedHortas(prevHortas =>
                prevHortas.map(h => (h.idHorta === hortaId ? { ...h, statusHorta: originalStatus } : h))
            );
        }
    };
    ...
    const filteredHortas = allFetchedHortas.filter(horta => {
        ...
        const matchesSearch =
            ...
            (horta.nomeUsuario || '').toLowerCase().includes(searchLower) ||
            (horta.statusHorta || '').toLowerCase().includes(searchLower);

        if (statusFilter === 'TODOS') return matchesSearch;
        if (statusFilter === 'ATIVOS') return matchesSearch && horta.statusHorta === 'ATIVA';
        if (statusFilter === 'INATIVOS') return matchesSearch && horta.statusHorta === 'INATIVA';
        if (statusFilter === 'VISITA_AGENDADA') return matchesSearch && horta.statusHorta === 'VISITA_AGENDADA';
        return false;
    }
    );
    ...
    const sortOptions = [
        ...
        { label: 'Proprietário', key: 'nomeUsuario' },
        { label: 'Endereço', key: 'endereco' },
        { label: 'Status', key: 'statusHorta' },
    ];
                ...
                {!isLoading && sortedHortas.length > 0 ? (
                    <section className="space-y-3">
                        {sortedHortas.map((horta) => {
                            const currentStatusDisplay = getStatusDisplayProps(horta.statusHorta);
                            return (
                                <div key={horta.idHorta} className="bg-[#E6E3DC] rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-300">
                                    <div
                                        ...
                                        onClick={() => toggleExpand(horta.idHorta)}
                                    >
                                        ...
                                        <div className="flex items-center space-x-2 flex-shrink-0 self-start xs:self-center">
                                            ...
                                            <FiChevronDown className={`w-4 h-4 text-gray-700 transform transition-transform ${expandedId === horta.idHorta ? 'rotate-180' : ''}`} />
                                        </div>
                                    </div>

                                    {expandedId === horta.idHorta && (
                                        <div className="border-t px-3 pt-2 pb-3 sm:px-4 bg-[#E6E3DC] rounded-b-lg space-y-2 text-xs text-gray-800">
                                            <p><strong className="font-medium">Proprietário:</strong> {horta.nomeUsuario || 'N/A'}</p>
                                            <p><strong className="font-medium">Tipo da Horta:</strong> {horta.nomeTipoDeHorta || 'N/A'}</p>
                                            <div className="pt-2 border-t border-gray-300 flex flex-row items-center justify-end gap-2 mt-2">
                                                <div ref={itemDropdownRef} className="relative">
                                                    <button
                                                        type="button"
                                                        onClick={(e) => { e.stopPropagation(); setActiveItemDropdownId(activeItemDropdownId === horta.idHorta ? null : horta.idHorta); }}
                                                    ...
                                                    {activeItemDropdownId === horta.idHorta && (
                                                        <div className="origin-top-right absolute right-0 mt-1 w-44 rounded-md shadow-lg bg-[#E6E3DC] ring-1 ring-black ring-opacity-5 z-10">
                                                            <div className="py-0.5">
                                                                {CHANGEABLE_STATUS_OPTIONS_FOR_MANAGEMENT.filter(opt => opt.value !== horta.statusHorta).map(option => (
                                                                    <button
                                                                        key={option.value}
                                                                        onClick={(e) => { e.stopPropagation(); handleStatusChange(horta.idHorta, option.value); }}
                                                ...
                                                <Link
                                                    to={`/app/hortas-editar/${horta.idHorta}`}
                                                ...
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); navigate(`/app/tela-de-descricao-de-solicitacao-hortas/${horta.idHorta}`); }}

...
import { 
  ...
  FiCheckCircle, FiXCircle, FiEye, FiInbox, FiClock,
...
// 1. IMPORTAMOS O HOOK CUSTOMIZADO
import { useManageableHortas } from '../../../hooks/useManageableHortas';

// --- Constantes e Funções Utilitárias (permanecem as mesmas) ---
...
export default function TelaGerenciamentoHortas() {
  ...
  // 2. LÓGICA DE DADOS DELEGADA AO HOOK
  // Renomeamos as variáveis na desestruturação para manter a compatibilidade com o resto do código.
  const { 
    hortas: allFetchedHortas, 
    isLoading, 
    error, 
    refetch: fetchAllManageableHortas, 
    setHortas: setAllFetchedHortas 
  } = useManageableHortas();

  // --- Estado da UI (permanece no componente) ---
  ...
  // A lógica de fetch e o useEffect associado foram removidos daqui e movidos para o hook.

  // Este useEffect para o controle do dropdown permanece, pois é uma lógica de UI.
  useEffect(() => {
  ...
  // Esta função permanece, mas agora usa 'setAllFetchedHortas' do hook.
  ...
  // --- Lógica de filtragem e ordenação (permanece a mesma) ---
  ...
  // --- Renderização do JSX (usando os estados do hook) ---
  if (isLoading) { 
    return (
    ...
    return (
      <div className="flex flex-col min-h-screen bg-[#A9AD99]">
        ...
        <div className="flex-grow flex flex-col items-center justify-center text-center p-6 m-4 sm:m-6 bg-[#E6E3DC] rounded-xl shadow-xl">
          ...
          <button
            onClick={fetchAllManageableHortas} // Ação de "Tentar Novamente" chama a função do hook
      ...
      <main className="container mx-auto p-3 sm:p-4 md:p-6 pt-4 sm:pt-6">
          ...
          <div className="space-y-4"> 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
              <div className="w-full">
                ...
                <select id="status-filter" value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full h-10 px-3 border border-gray-300 rounded-lg bg-[#E6E3DC] text-sm focus:ring-1 focus:ring-green-600 text-gray-900">
              ...
              <div className="w-full">
                ...
                <select id="sort-by" value={sortConfig.key} onChange={(e) => requestSort(e.target.value)} className="w-full h-10 px-3 border border-gray-300 rounded-lg bg-[#E6E3DC] text-sm focus:ring-1 focus:ring-green-600 text-gray-900">
                  {sortOptions.map(option => (<option key={option.key} value={option.key}>{option.label}</option>))}
                </select>
              </div>
              <button onClick={() => requestSort(sortConfig.key)} className="w-full sm:w-auto h-10 px-3 bg-[#E6E3DC] text-gray-800 border border-gray-300 rounded-lg flex items-center justify-center gap-1 hover:bg-[#e0dbcf] text-sm" title={sortConfig.direction === 'ascending' ? "Mudar para Descendente" : "Mudar para Ascendente"}>
                {sortConfig.direction === 'ascending' ? <FiArrowUp className="w-4 h-4" /> : <FiArrowDown className="w-4 h-4" />}
                <span className="sm:hidden">{sortConfig.direction === 'ascending' ? 'Cresc.' : 'Decresc.'}</span>
                <span className="hidden sm:inline">{sortConfig.direction === 'ascending' ? 'Crescente' : 'Decrescente'}</span>
              </button>
            </div>
          </div>
        </section>
        ...
        {!isLoading && sortedHortas.length > 0 ? (
          <section className="space-y-3">
            {sortedHortas.map((horta) => { 
              ...
              return (
                <div key={horta.id} className="bg-[#E6E3DC] rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-300">
                  <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between px-3 py-3 sm:px-4 cursor-pointer hover:bg-[#E0DBCF] rounded-t-lg" onClick={() => toggleExpand(horta.id)}>
                    <div className="flex-1 min-w-0 mb-2 xs:mb-0 xs:mr-2">
                      ...
                      <p className="text-[11px] xs:text-xs text-gray-700 truncate" title={horta.endereco}><FiMapPin className="inline w-3 h-3 mr-1 text-gray-700" />{horta.endereco}</p>
                    </div>
                    <div className="flex items-center space-x-2 flex-shrink-0 self-start xs:self-center">
                      <div className={currentStatusDisplay.className} title={`Status: ${currentStatusDisplay.label}`}>{currentStatusDisplay.icon}<span className="hidden xs:inline">{currentStatusDisplay.label}</span></div>
                    ...
                    <div className="border-t px-3 pt-2 pb-3 sm:px-4 bg-[#E6E3DC] rounded-b-lg space-y-2 text-xs text-gray-800">
                      ...
                      <div className="pt-2 border-t border-gray-300 flex flex-row items-center justify-end gap-2 mt-2">
                        <div ref={el => itemDropdownRef.current = el} className="relative">
                          <button type="button" onClick={(e) => { e.stopPropagation(); setActiveItemDropdownId(activeItemDropdownId === horta.id ? null : horta.id); }} className="inline-flex justify-center items-center rounded-md border border-gray-300 shadow-sm px-2.5 py-1 bg-[#E6E3DC] text-[10px] xs:text-xs font-medium text-gray-900 hover:bg-[#e0dbcf]">Mudar Status <FiChevronDown className="ml-1 h-3.5 w-3.5" /></button>
                            ...
                            <div className="origin-top-right absolute right-0 mt-1 w-44 rounded-md shadow-lg bg-[#E6E3DC] ring-1 ring-black ring-opacity-5 z-10">
                              <div className="py-0.5">
                                {CHANGEABLE_STATUS_OPTIONS_FOR_MANAGEMENT.filter(opt => opt.value !== horta.status).map(option => (
                                  <button key={option.value} onClick={(e) => { e.stopPropagation(); handleStatusChange(horta.id, option.value); }} className={`group flex items-center w-full px-2.5 py-1.5 text-[10px] xs:text-xs text-left text-gray-800 ${option.hoverClass}`}>{React.cloneElement(option.icon, { className: 'mr-1.5 h-3.5 w-3.5' })}{option.label}</button>
                                ))}
                              </div>
                            </div>
                          )}
                        </div>
                        <Link to={`/app/hortas-editar/${horta.id}`} onClick={(e) => e.stopPropagation()} className="flex items-center justify-center gap-1 px-2 py-1 bg-green-600 text-white rounded-md hover:bg-green-700 text-xs font-medium"><FiEdit3 className="w-3 h-3" /> <span className="hidden xs:inline">Editar</span></Link>
                        <button onClick={(e) => { e.stopPropagation(); navigate(`/app/tela-de-descricao-de-solicitacao-hortas/${horta.id}`); }} className="flex items-center justify-center gap-1 px-2 py-1 bg-green-800 text-white rounded-md hover:bg-green-900 text-xs font-medium"><FiEye className="w-3 h-3" /> <span className="hidden xs:inline">Ver</span></button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="text-center py-16 bg-[#E6E3DC] rounded-xl shadow-lg">
                ...
                <p className="text-gray-800 mt-2 text-sm">{searchQuery ? "Nenhuma horta corresponde à sua busca." : "Não há hortas para exibir com os filtros atuais."}</p>
            </div>
        )}
        ...
        {!isLoading && sortedHortas.length > 0 && ( 
            <footer className="mt-8 sm:mt-10 flex justify-center items-center space-x-1 sm:space-x-2 p-4 bg-[#E6E3DC] rounded-xl shadow-lg">
                <button className="p-2 rounded-full hover:bg-[#e0dbcf] text-gray-700 disabled:opacity-50 cursor-not-allowed" disabled><FiChevronLeft className="w-5 h-5" /></button>
                <span className="text-sm font-semibold text-green-800 px-3.5 py-1.5 bg-green-100 rounded-md">1</span>
                <button className="p-2 rounded-full hover:bg-[#e0dbcf] text-gray-700 disabled:opacity-50 cursor-not-allowed" disabled><FiChevronRight className="w-5 h-5" /></button>
            </footer>
        )}
      </main>
    </div>

...
// --- CORREÇÃO: DEFINIÇÃO DOS COMPONENTES AUXILIARES DE UI ---
...
export default function TelaEdicaoHorta() {
  ...
  if (isLoading || (optionsLoading && hortaIdFromParams)) return <div className="flex flex-col min-h-screen items-center justify-center p-4"><FiLoader className="animate-spin text-5xl text-green-600" /><p className="mt-4 text-lg text-gray-700">Carregando dados...</p></div>;
  if (error) return <div className="flex flex-col min-h-screen items-center justify-center p-4 text-center"><FiAlertCircle className="text-5xl text-red-500 mb-3" /><p className="text-lg font-semibold text-red-600">Erro ao Carregar</p><span className="text-red-500">{error}</span><button onClick={() => navigate(-1)} className="mt-6 px-5 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition">Voltar</button></div>;
  if (hortaIdFromParams && !originalHorta && !isLoading) return <div className="flex flex-col min-h-screen items-center justify-center p-4 text-center"><FiAlertCircle className="text-5xl text-orange-500 mb-3" /><p className="text-lg font-semibold text-orange-600">Horta não encontrada</p><span className="text-orange-500">A horta com o ID fornecido não pôde ser carregada.</span><button onClick={() => navigate(-1)} className="mt-6 px-5 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition">Voltar</button></div>;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 font-poppins">
      ...
      <div className="flex-grow flex flex-col">
          ...
          <form onSubmit={handleSubmit} id="edit-horta-form" className="max-w-4xl mx-auto space-y-6">
            <header className="pb-6 border-b border-gray-200 flex justify-between items-center">
              <div>
                ...
                <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Editando Horta: <span className="text-green-600">{originalHorta?.nomeHorta}</span></h1>
                <p className="text-sm text-gray-600 mt-1">Modifique as informações da horta abaixo.</p>
              </div>
              <button type="button" onClick={handleResetForm} disabled={isSubmitting || !originalHorta} className="p-2 text-gray-500 hover:text-blue-600 disabled:opacity-50" title="Restaurar dados originais"><FiRotateCcw className="w-5 h-5" /></button>
            </header>
            ...
            <Section title="Informações Gerais">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <div>
                  ...
                  <input id="nomeHorta" name="nomeHorta" type="text" value={formData.nomeHorta} onChange={handleChange} required className="mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" />
                </div>
                <div>
                  <label htmlFor="statusHorta" className="block text-sm font-medium text-gray-700">Status da Horta *</label>
                  <select id="statusHorta" name="statusHorta" value={formData.statusHorta} onChange={handleChange} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
            ...
            <Section title="Detalhes da Área e Grupo">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    <div>
                        ...
                        <input id="tamanhoAreaProducao" name="tamanhoAreaProducao" type="number" step="0.1" min="0" value={formData.tamanhoAreaProducao} onChange={handleChange} required className="mt-1 w-full rounded-md border-gray-300 shadow-sm" />
                    </div>
                    <div>
                        <label htmlFor="qntPessoas" className="block text-sm font-medium text-gray-700">Nº de Pessoas Envolvidas *</label>
                        <input id="qntPessoas" name="qntPessoas" type="number" min="1" value={formData.qntPessoas} onChange={handleChange} required className="mt-1 w-full rounded-md border-gray-300 shadow-sm" />
                    </div>
                    <div className="md:col-span-2">
                        <label htmlFor="caracteristicaGrupo" className="block text-sm font-medium text-gray-700">Características do Grupo</label>
                        <textarea id="caracteristicaGrupo" name="caracteristicaGrupo" value={formData.caracteristicaGrupo} onChange={handleChange} rows="3" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
                    </div>
                    <div className="md:col-span-2">
                        <label htmlFor="atividadeDescricao" className="block text-sm font-medium text-gray-700">Descrição das Atividades *</label>
                        <textarea id="atividadeDescricao" name="atividadeDescricao" value={formData.atividadeDescricao} onChange={handleChange} required rows="3" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
                    </div>
                </div>
            </Section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 items-start">
                <div className="space-y-4">
                  <div>
                    ...
                    <select name="idTipoDeHorta" id="idTipoDeHorta" value={formData.idTipoDeHorta} onChange={handleChange} required disabled={optionsLoading} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                  ...
                  <div>
                    ...
                    <select name="idUsuario" id="idUsuario" value={formData.idUsuario} onChange={handleChange} required disabled={optionsLoading} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                  ...
                  <div>
                    ...
                    <select name="idUnidadeEnsino" id="idUnidadeEnsino" value={formData.idUnidadeEnsino} onChange={handleChange} required disabled={optionsLoading} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                  ...
                  <div>
                    ...
                    <select name="idAreaClassificacao" id="idAreaClassificacao" value={formData.idAreaClassificacao} onChange={handleChange} required disabled={optionsLoading} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                  ...
                  <div>
                    ...
                    <select name="idAtividadesProdutivas" id="idAtividadesProdutivas" value={formData.idAtividadesProdutivas} onChange={handleChange} required disabled={optionsLoading} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                ...
                <div className="space-y-2">
                  ...
                  <label htmlFor="imagem-upload-horta" className="w-full cursor-pointer flex items-center justify-center px-4 py-2.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors text-sm font-medium shadow-sm"><FiUpload className="w-4 h-4 mr-2" /><span>Alterar Imagem</span></label>
        ...
        <footer className="flex-shrink-0 bg-white shadow-lg p-4 sticky bottom-0">
          <div className="max-w-4xl mx-auto flex justify-end gap-3">
            <button type="button" onClick={() => navigate(-1)} disabled={isSubmitting} className="px-5 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Cancelar</button>

// Caminho: src/modules/relatorios/TelaDeRelatorios.jsx
import React, { useState, useMemo, useRef, useCallback, useEffect } from "react";
import { FaPlus, FaEdit, FaDownload, FaSpinner, FaSearch, FaLeaf, FaChalkboardTeacher } from "react-icons/fa";
...
const SORT_OPTIONS = [ { label: "Data Recente", key: "data" }, { label: "Nome (A-Z)", key: "nome" } ];

const StatusBadge = ({ status }) => {
  const statusStyles = {
    'Ativo': 'bg-green-100 text-green-700', 'Disponível': 'bg-blue-100 text-blue-700',
    'Desativado': 'bg-gray-100 text-gray-700', 'ATIVA': 'bg-green-100 text-green-700',
    'PENDENTE': 'bg-yellow-100 text-yellow-700', 'INATIVA': 'bg-red-100 text-red-700',
    'VISITA_AGENDADA': 'bg-sky-100 text-sky-700',
  };
  const statusText = typeof status === 'string' ? status.replace(/_/g, ' ') : (status || "—");
  return <span className={`font-medium px-2.5 py-1 rounded-full text-xs capitalize ${statusStyles[status] || 'bg-purple-100 text-purple-800'}`}>{statusText}</span>;
};
...
const RelatorioCard = ({ item }) => {
  ...
  const handleNavigation = () => {
    let path;
    if (item.isCurso) path = `/app/tela-de-edicao-de-cursos/${item.id}`;
    else if (item.tipo === "Horta") path = `/app/tela-de-descricao-de-solicitacao-hortas/${item.id}`;
    else path = `/app/editar-relatorio/${item.id}`;
    navigate(path);
  };
  ...
  return (
    <div className="flex flex-col justify-between bg-white p-4 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 transform hover:-translate-y-1">
      <div>
        <div className="flex items-center mb-2"><IconComponent className={`w-5 h-5 mr-2.5 ${iconColor}`} /><p className="font-semibold text-gray-800 break-words leading-tight text-base" title={item.nome}>{item.nome}</p></div>
        <p className="text-gray-500 text-xs mb-1">{item.tipo} – {formatDate(item.data)}</p>
        <div className="text-gray-600 text-sm mt-2 flex items-center gap-1.5"><span className="text-xs font-medium">Status:</span><StatusBadge status={item.status} /></div>
      </div>
      <div className="mt-4 pt-3 border-t border-gray-200 flex justify-end"><button onClick={handleNavigation} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors"><FaEdit className="w-4 h-4" /></button></div>
    </div>
  );
};
...
export default function TelaDeRelatorios() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const [cursosResponse, hortasResponse] = await Promise.allSettled([api.get('/cursos'), api.get('/hortas')]);
        let combinedData = [];
        if (cursosResponse.status === 'fulfilled') {
          combinedData.push(...cursosResponse.value.data.map(c => ({ id: c.idCurso, nome: c.nome, tipo: c.tipoAtividade || "Curso", data: c.dataInicio, status: c.ativo ? 'Ativo' : 'Desativado', isCurso: true })));
        } else { toast.error("Falha ao carregar dados dos cursos."); }
        if (hortasResponse.status === 'fulfilled') {
          combinedData.push(...hortasResponse.value.data.map(h => ({ id: h.idHorta, nome: h.nomeHorta, tipo: "Horta", data: h.dataCriacao, status: h.statusHorta, isCurso: false })));
        } else { toast.error("Falha ao carregar dados das hortas."); }
        combinedData.push({ id: 'modelo-1', nome: "Modelo - Horta Acompanhamento", tipo: "Modelo", data: "2024-01-15", status: "Disponível", isCurso: false });
        setData(combinedData);
      } catch (err) { setError("Não foi possível carregar os dados."); toast.error("Erro geral ao carregar dados."); } 
  ...
  const filteredData = useMemo(() => {
    let items = data;
    if (activeFilter !== "Todos") items = items.filter(item => item.tipo === activeFilter);
    if (searchTerm) items = items.filter(item => item.nome.toLowerCase().includes(searchTerm.toLowerCase()));
    return items;
  ...
  const handleDownload = useCallback(async (url, defaultFilename, setIsDownloadingState) => {
    ...
    try {
      ...
      toast.error(`Não foi possível gerar o relatório: ${defaultFilename}.`);
    } finally {
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg p-6">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="relative" ref={optionsMenuRef}>
            <button onClick={() => setShowOptions(o => !o)} className="bg-green-600 hover:bg-green-700 text-white p-2.5 rounded-full transition shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" aria-label="Ações"><FaPlus className="w-5 h-5" /></button>
        ...
        <section className="mb-6 space-y-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-x-6 gap-y-4">
            <div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-gray-700 text-sm shrink-0">Filtrar por:</span>{FILTER_TYPES.map(type => (<button key={type} onClick={() => setActiveFilter(type)} className={`px-3.5 py-1.5 text-sm rounded-full transition-colors duration-200 border ${activeFilter === type ? 'bg-green-600 text-white font-semibold shadow-md border-green-700' : 'bg-white text-gray-700 hover:bg-gray-100 border-gray-300 hover:border-gray-400'}`}>{type}</button>))}</div>
            <div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-gray-700 text-sm shrink-0">Ordenar por:</span>{SORT_OPTIONS.map(opt => (<button key={opt.key} onClick={() => setSortKey(opt.key)} className={`px-3.5 py-1.5 text-sm rounded-full transition-colors duration-200 border ${sortKey === opt.key ? 'bg-gray-700 text-white font-semibold shadow-md border-gray-800' : 'bg-white text-gray-700 hover:bg-gray-100 border-gray-300 hover:border-gray-400'}`}>{opt.label}</button>))}</div>
          </div>
        </section>

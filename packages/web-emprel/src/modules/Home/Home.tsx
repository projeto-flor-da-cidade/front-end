...
const PLACEHOLDER_FILENAME = "folhin.png";

const getCourseImageUrl = (fotoBanner?: string) => {
  if (fotoBanner && fotoBanner !== PLACEHOLDER_FILENAME) {
    return `${API_BASE_URL}/uploads/banners/${fotoBanner}`;
  }
    ...
    const fetchCourses = async () => {
      try {
        ...
        if (err instanceof Error) {
          setError(`Falha ao carregar cursos: ${err.message}. Verifique a conexão com a API.`);
        } else {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {serviceCards.map((card, i) => (
              <a
                ...
                className="group flex flex-col overflow-hidden rounded-2xl bg-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-2xl"
              >
                ...
                <div className="flex flex-1 flex-col bg-[#e7eff6] justify-between p-5">
                  ...
                  <span className="self-start rounded-full bg-[#F4D35E] px-5 py-2 font-bold text-sm text-[#1D3557] transition-colors duration-300 group-hover:bg-[#FFE46B]">
                    Saiba mais
                  </span>
                </div>
              </a>
